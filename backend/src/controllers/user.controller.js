...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    throw new ApiError(
      500,
      "Something went wrong while generating refresh anmd access token"
    );
  }
};
...
const logoutUser = asyncHandler(async (req, res) => {
  await User.findByIdAndUpdate(
    ...
    {
      $set: {
        refreshToken: undefined, //if not done with this, use $unset:{refreshToken: 1}
      },
    },
    {
      new: true,
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  const channel = await User.aggregate([
    ...
    //We count scuscriber of his Channel(Through Channel)
    {
      $lookup: {
        from: "subscriptions",
        localField: "_id",
    ...
    //Number of channel is subscribed by you(Through Subscribers)
    {
      $lookup: {
        from: "subscriptions",
        localField: "_id",
    ...
    //We added some feilds in original user object
    {
      $addFields: {
        subscribersCount: {
          $size: "$subscribers",
